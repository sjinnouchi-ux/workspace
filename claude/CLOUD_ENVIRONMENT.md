...
- 各行を独立させ `|| true` を付ける。一行連結（`apt update && apt install ... || true`）は `apt update` 失敗時にinstall全体がスキップされるため使用しない（2026-07-20の初回検証で `gh` 未導入となった原因）。
...
- [x] GitHub App承認（All repositories・2026-07-20完了）
- [x] 環境 `yumekango-standard` 作成（Trusted / 環境変数なし / Setup script設定・2026-07-20完了）
- [x] 既定環境に設定（2026-07-20完了）
- [x] Verificationセッション実行（2026-07-20: SHA取得・Canonical Entry 23件・rg・proxy-injected成功。ghのみ旧script起因で未導入→script修正済み、次回セッションのキャッシュ再構築で導入される）
- [ ] デスクトップPC・ノートPC両方から同一セッションを開けることを確認
- [x] 各プロジェクトrepoへ `CLAUDE.md`（`@AGENTS.md`）追加（2026-07-20完了）
