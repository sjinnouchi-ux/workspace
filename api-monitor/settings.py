...
import db
from api_clients import MODEL_PRICING, SUPPORTED_SERVICES
...
def render() -> None:
    ...
    st.warning("APIキーはローカルSQLiteに保存されます。GitHubやNotionには保存しないでください。")
        ...
        with st.expander(SERVICE_LABELS[service], expanded=False):
            ...
            if cols[0].button("保存", key=f"save_{service}"):
                if new_key.strip():
                    ...
                    st.success("保存しました。再読み込み後に反映されます。")
                else:
    ...
    st.caption("プロジェクト名、利用モデル、用途メモを登録して、あとから費用を見やすくします。")
