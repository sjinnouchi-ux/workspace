...
タイトル：AI経営実装度診断
サブタイトル：会社としてAIを管理・実装できる状態にあるかを確認する簡易診断です。
...
- D AI窓口あり・現場実装準備フェーズ：AIの窓口が見えてきています。エージェントやLLMを現場で使える形に整える段階ではありませんか？
...
実際の文章はスプレッドシートから差し替え可能にする。初期値の全文は `config.seed.json` の `result_steps`（20行）に格納済み。各結果の構成は「10=声／20=現在の状態／30=起きやすいこと／40=最初に決めるべきこと」。
