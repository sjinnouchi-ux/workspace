...
export default {
  async fetch(request, env, ctx) {
    ...
    if (shouldTryKAlert(payload)) {
      try {
        const kAlertRes = await forwardJson(env.K_ALERT_GAS_URL, body);
        const decision = await safeResponseJson(kAlertRes);
    ...
    ctx.waitUntil(forwardJson(env.LEGACY_GAS_URL, body));
    return new Response("OK", { status: 200 });
  }
};

async function handleLiffForm(env) {
  let categories = [];
  try {
    const res = await fetch(env.LEGACY_GAS_URL + "?action=getCategories", { redirect: "follow" });
  ...
  const currentMonth = tokyoDate.getMonth() + 1;
  const html = buildFormHtml(categories, currentMonth, env.LEGACY_GAS_URL);
...
function shouldTryKAlert(payload) {
  ...
  return payload.events.some((event) => {
    ...
    return event.message.text.trim().length > 0;
  });
}

async function forwardJson(url, body) {
  if (!url) {
...
function buildFormHtml(categories, currentMonth, gasUrl) {
