...
公式LINEから匿名報告を受け付け、AIが報告内容を `いつ`, `どこで`, `だれが`, `なにを`, `どのように` に分解し、不足項目だけを短く聞き返すテスト環境を作る。
...
- 開始トリガーとして `Kアラート`, `匿名報告`, `匿名報告開始`, `報告する` に対応
- リッチメニュー想定の開始文に対し、匿名報告の案内文を返信
