...
- [ ] Anthropic APIで初回コメントを構造化できることを確認
- [x] 不足項目の質問文を生成できることを確認

補足:

- `SPREADSHEET_ID`, `LINE_CHANNEL_ACCESS_TOKEN` は設定済み
- Anthropicへ切り替える場合は `AI_PROVIDER=anthropic`, `ANTHROPIC_MODEL=claude-haiku-4-5`, `ANTHROPIC_API_KEY` を設定する
...
- [ ] Anthropic APIキー設定後、不足項目のLINE返信
- [ ] 追加回答で既存行を更新
- [ ] 完了メッセージ返信を実機確認
