...
|---|---|---|---|---|---|---|---|
| 税理士事務所DX | 税理士DX, zeirishi-office-dx | Active / Planning | 未定 | 未定 | `https://github.com/sjinnouchi-ux/zeirishi-office-dx` | `AGENTS.md`; `README.md`; `PROJECT_BRIEF.md`; `docs/secret-management.md`; `docs/freee-access.md`; `IMPLEMENTATION_LOG.md` | `yumekango.com` ではない別Googleアカウントを使用し、全タスクで実作業前のログイン確認が必須。アカウント識別子はprivate repoを正本とする。freeeはログイン画面到達・認証未実施。GCP projectは未確定、Secret Managerは未構築。要件、対象業務、技術構成、設計・実装担当は未確定。 |
