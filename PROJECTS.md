...
| Kアラート本番開発 | Kアラート, K-alert, kalert, k-alert-production | Active | Claude | Codex | `https://github.com/sjinnouchi-ux/k-alert-production` | `AGENTS.md`; `PROJECT_BRIEF.md`; `DESIGN_LOG.md`; `IMPLEMENTATION_LOG.md`; `docs/reports/2026-06-30-k-alert-production-current-status.md` | 本番運用あり。runtime secretはSecret Manager/Worker storeで確認済み。Cloudflare deploy manifestと限定IAMは整備済みだが、専用accountのscoped token登録までは `blocked_pending_token`。共通tokenやWrangler OAuthへフォールバックしない。 |
