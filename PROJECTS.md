...
| Kアラート本番開発 | Kアラート, K-alert, kalert, k-alert-production | Active | Claude | Codex |  | 2026-07-01 | `C:\Users\irodo\Documents\Kアラート・本番開発` | 本番運用あり。Cloud Run / Cloudflare Worker / GAS / Supabase。 |
| 台湾プロジェクト | 台湾, taiwan, taiwan-outreach, インバウンド | Active | Claude | Codex |  | 2026-07-01 | `C:\Users\irodo\Documents\台湾プロジェクト` | 看護守台湾向けLP/SNS/YouTube/GA4/Search Console。 |
| dori-manga | どり漫画, dori, どり看護師 | Active（移行中） | Claude | Codex |  |  | `dori-manga/` | 既存workspace配下から新方式へ移行予定。 |
| API Monitor | APIモニター, api-monitor | Active（移行検討） | Claude | Codex |  |  | `api-monitor/` | 既存未コミット差分あり。今回は中身を変更しない。 |
...
- Statusは `Active` / `Active / Pilot` / `Active（移行中）` / `Frozen` / `Completed` / `Legacy` を使う。
