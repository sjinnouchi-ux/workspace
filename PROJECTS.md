...
|---|---|---|---|---|---|---|---|
| 税理士事務所DX | 税理士DX, zeirishi-office-dx | Active / Planning | 未定 | 未定 | `https://github.com/sjinnouchi-ux/zeirishi-office-dx` | `AGENTS.md`; `README.md`; `PROJECT_BRIEF.md`; `docs/secret-management.md`; `docs/freee-access.md`; `docs/freee-mcp.md`; `IMPLEMENTATION_LOG.md` | `yumekango.com` ではない別Googleアカウントを使用し、全タスクで実作業前のログイン確認が必須。アカウント識別子はprivate repoを正本とする。freee公式Remote MCPはプロジェクト専用名でOAuth接続済み。個人用freeeは将来の別ルートとし流用禁止。GCP projectは作成済み、Cloud BillingリンクとSecret Manager API有効化まで完了。登録済みSecretは0件で `api_enabled_no_secrets`。要件、対象業務、技術構成、設計・実装担当は未確定。 |
