...
|---|---|---|---|---|---|---|---|
| 税理士事務所DX | 税理士DX, zeirishi-office-dx | Active / Planning | 未定 | 未定 | `https://github.com/sjinnouchi-ux/zeirishi-office-dx` | `AGENTS.md`; `README.md`; `PROJECT_BRIEF.md`; `docs/secret-management.md`; `docs/freee-access.md`; `docs/freee-mcp.md`; `docs/supabase-access.md`; `docs/box-receipt-source.md`; `IMPLEMENTATION_LOG.md` | `yumekango.com` ではない別Googleアカウントを使用し、全タスクで実作業前のログイン確認が必須。アカウント識別子はprivate repoを正本とする。freee公式Remote MCPはプロジェクト専用名でOAuth接続済み。個人用freeeは将来の別ルートとし流用禁止。GCP project、Cloud Billing、Secret Managerを構築済み。領収書管理Supabase Projectを東京RegionのFree Planで作成し、3 Secretを登録。GitHub Actions Pythonの6時間keepaliveは実疎通済み。現在のBoxフォルダはクライアント領収書スキャンの受領元として確認済み。個別ファイル読取り、Box API、OCR、Box→Supabase/freee連携、領収書用テーブル・カラムは未設計・未実施。既存の別Supabase設定は流用禁止。その他の要件、対象業務、技術構成、設計・実装担当は未確定。 |
