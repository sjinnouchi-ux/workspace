...
| dori-manga | どり漫画, dori, どり看護師 | Active | Claude | Codex | `https://github.com/sjinnouchi-ux/workspace/tree/main/dori-manga` | `dori-manga/CLAUDE.md`; `dori-manga/docs/work_log.md`; `dori-manga/docs/secret-management.md` | 独立repoではなく `workspace` 内の正本パス。空の同名ローカルGitを入口にしない。Secretは保管済みだが、Cloudflare deployの新PC用manifest/IAMは未整備のため現在blocked。 |
