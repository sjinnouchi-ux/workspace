...
|---|---|---|---|---|---|---|---|---|
| 家計簿LIFF FastAPI化 | 家計簿, kakeibo, kakeibo-liff, yumekango-worker | Active / Pilot | FABLE 5 / Claude | Codex | 2026-07-02 | 2026-07-02 | `https://github.com/sjinnouchi-ux/kakeibo-liff` | Phase 1 FastAPI/Worker骨格を `codex/phase-1-fastapi-worker` に実装。既存LIFF/GAS/Workerを段階的にFastAPI化する。 |
...
| AI経営実装度診断WEBアプリ | 経営者AI診断, 経営者AI診断アプリ, AI経営診断, ai-keiei-shindan | Active | Claude / Opus 4.8 | Codex | 2026-07-04 | 2026-07-04 | `ai-keiei-shindan/` | 個人営業マンが経営者の前で使う限定URLの診断アプリ。GitHub Pages + GAS + Google Sheets。ロジック仕様書 `ai_keiei_shindan_app_spec.md` の所在確認が初回実装前の停止条件。 |
...
| Kアラート・テスト開発 | k-alert-test, Kアラートテスト | Archived / Legacy | 2026-07-09 | なし | 過去にテスト版が存在したことだけを残す。詳細なコード、GAS、Sheets、Worker、LIFF記録は破棄済み。現行はKアラート本番開発を参照。 |
