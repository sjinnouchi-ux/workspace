...
| API Monitor | APIモニター, api-monitor | Active | Claude | Codex | `https://github.com/sjinnouchi-ux/api-monitor` | `AGENTS.md`; `PROJECT_BRIEF.md`; `DESIGN_LOG.md`; `IMPLEMENTATION_LOG.md`; `docs/secret-management.md` | 独立プロジェクトGitへ移行済み。Secret Managerのruntime/sync role、限定IAM、provider疎通、ローカル平文保存撤去まで完了。新PCは管理ターミナルhelper経由で起動する。Google Billing同期は非秘密設定と各PCのADC確認が必要。 |
...
| 経営者労基診断アプリ開発 | 労基診断, 経営者労基診断, 労務リスク診断, keieisha-rouki-shindan | Active / Planning | Claude / Codex | Codex | `https://github.com/sjinnouchi-ux/keieisha-rouki-shindan` | `AGENTS.md`; `README.md`; `PROJECT_BRIEF.md`; `IMPLEMENTATION_LOG.md` | 5問YES/NOの労務リスク診断とクライアント説明資料。ユーザー提供PDFを原典として質問・配点・助言・UIを設計する。 |
...
| 管理ターミナル | management-terminal, mgmt-terminal | Support / Workflow | Claude | Codex | `https://github.com/sjinnouchi-ux/mgmt-terminal` | `docs/DATABASE_SCHEMA.md`; `docs/superpowers/specs/2026-07-15-finance-foundation-design.md`; `docs/superpowers/plans/2026-07-15-finance-foundation-roadmap.md`; `docs/reports/2026-07-15-finance-core-implementation-log.md` | 2026-07-06の設計相談メモ・引継ぎメモは履歴資料。Migration 1のlinked Supabase適用状態は実装ログを正本とし、Migration 2AまではHTTP/API権限を変更しない。 |
