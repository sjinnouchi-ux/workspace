...
Codexは最初に `ambiguous` を除外し、該当しない依頼だけを継続意図の有無で分類する。`ambiguous` に該当せず、継続を示す明示的な表現がないShogun操作依頼は新規と扱う。これにより `new`、`resume`、`ambiguous` を相互排他的にする。Codexは次の意味を持つ標準ガードを依頼本文の先頭へ付加する。
