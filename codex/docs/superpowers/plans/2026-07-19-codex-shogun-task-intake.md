...
- [x] **Step 1: startupへversioned intake blockを追加する**
...
- [x] **Step 2: Custom Instructionsの貼付文面へbootstrapを追加する**
...
- [x] **Step 2: planの完了checkboxと実測結果を更新する**
...
Expected: 7 + 19 + 11 tests passed、failed 0、skipped 0、diff check出力なし。statusには意図したwork logとplan変更だけが表示される。

- [x] **Step 4: secretsとscopeを確認する**
...
- [x] **Step 5: work logとplanをcommitする**
...
PR本文へ、base SHA、目的、変更ファイル、7/19/11のtest実数、failed/skip 0、独立review、既知制約、rollback、Shogun runtime/WebUI/Custom Instructions未変更を記載する。

- [x] **Step 4: PRとlocal stateを確認する**
...
- Contract tests: 7 passed / 0 failed / 0 skipped
- Existing Python hook tests: 19 passed / 0 failed / 0 skipped
- Existing PowerShell installer tests: 11 passed / 0 failed / 0 skipped
- `git diff --check`: passed
- Changed-file scope: 6 expected files / 0 unexpected files
- Secret-like material scan: 0 findings
- Independent review: 1 Important finding fixed / 0 remaining findings
- Draft PR: #32 / GitHub status checks not configured
- Shogun runtime deployment: not performed
- Local Custom Instructions update: not performed
