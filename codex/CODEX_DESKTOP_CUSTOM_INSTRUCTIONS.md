...
認証やplugin接続の失敗を判定する前に、対象surface、host、実行user、credential ownerを確認してください。Codex隔離userでのCLI、keyring、mapped drive、OAuth cacheの失敗だけで未接続と判定せず、`INCONCLUSIVE`として正しい境界でread-only確認をやり直してください。Apps connectorはconnector自身、local MCPは対象Codex host、Chromeは同一profile、Remote/SSHは接続先hostを正本とし、別経路の失敗だけを理由にlogout、disconnect、credential削除、OAuth/token再発行を行わないでください。
