...
## 2026-07-14｜frontend-design共有スキル追加

- Anthropic公式`anthropics/skills`の`frontend-design`をcommit `9d2f1ae187231d8199c64b5b762e1bdf2244733d`で固定した。
- `SKILL.md`とApache License 2.0の`LICENSE.txt`を改変せず`codex/skills/frontend-design/`へ収録した。
- `SOURCE.md`に配布元、取得日、commit、各ファイルのSHA-256を記録した。
- 複数PCではGitHub上の正本からCodex system skill installerで導入する方針とした。
- GitHub認証失敗は、Codex隔離ユーザーから実ユーザーkeyringを参照できないことが原因だった。承認付き実ユーザー実行では`sjinnouchi-ux`のkeyring認証とADMIN権限を確認し、`gh auth setup-git --hostname github.com`を再適用した。秘密値は記録していない。

---

## 2026-07-14｜接続・認証境界の誤判定防止

- Windows隔離userのCLI失敗を未認証と断定しない三値判定`CONNECTED` / `DISCONNECTED` / `INCONCLUSIVE`を共通起動手順へ追加した。
- App connector、local CLI/sync、local/host MCP、hosted plugin MCP、browser/Chrome、Remote/SSHを別境界として監査するrunbookを追加した。
- GitHub確認scriptは隔離userで外部照会せずexit 2、実userでtokenを表示せずaccount/repo permissionだけを検証する設計とした。
- Google Drive connectorは接続成功、ローカル`G:`は未観測であり、両者が別境界であることを確認した。
- `gcloud`は隔離userのPATHでは未検出、実userではcommandとactive accountを確認し、CLI存在確認にも同じfalse negativeが起こることを確認した。account文字列やtokenは出力していない。
- ADCはcredential fileの存在だけでは有効性・主体・権限を判定できないため`INCONCLUSIVE`、`clasp`は実userでもcommand/user設定がなく未構成と整理した。
- Google Search Console MCPは最小read-only probe成功。GA4 MCPとbrowser/Chromeは現在のtaskにtool未露出のため、未認証ではなく`INCONCLUSIVE`とした。
- 別経路の失敗だけを理由にlogout、disconnect、credential削除、OAuth/token再発行を行わない禁止事項を追加した。
- このPCの`C:\Users\jinnouchi\.codex\AGENTS.md`を更新後のbootstrap文面へ同期し、SHA-256 `CBE9B2E4E066722627B5AFFEA1CE34E9047A2D291DE60767E796C8CB4A62D50C`の一致を確認した。更新前は`C:\Users\jinnouchi\.codex\backups\AGENTS-before-connection-boundary-20260714-093938.md`へ保存した。
- 秘密値、OAuthコード、token、認証JSON、private content、生ログは取得・記録していない。
