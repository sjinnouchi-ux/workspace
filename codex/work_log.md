...
## 2026-07-24 — Shogun Ops policy contract

- `workspace` base `9774b76025349c6b27e2e2f43b67075625a232c6` と、review済みShogun merge `d880a3d40b4af4dde62f6e912fccbbbd925d31fd` を前提に、`CODEX_SHOGUN_OPS_V1` を追加した。
- 変更ファイルは `codex/CODEX_DESKTOP_STARTUP.md`、`codex/CODEX_DESKTOP_CUSTOM_INSTRUCTIONS.md`、`codex/tests/test_shogun_ops_docs.py`、`codex/work_log.md` のみである。
- focused contract test `python codex/tests/test_shogun_ops_docs.py`（8 tests）と既存intake contract test `python codex/tests/test_shogun_task_intake_docs.py`（7 tests）、`git diff --check` を実行した。full suiteは実行しない。
- deployment、host policy、Ops snapshot、WSL2 runtime、Shogun start/restart/task deliveryは別の明示的checkpointであり、本変更では実施していない。

## 2026-07-24 — Shogun diagnostics per-call provenance relaxation

- 根本原因は、固定Ops wrapperのdeployment後もtask intakeがdelivery前にlegacy diagnosticsを要求し、実行前のsource-hash approvalが循環的に失敗していたことである。
- 固定Ops `status` をdeployed operation/deliveryの唯一のpreflightとし、direct legacy diagnosticsではper-callのGitHub registry、active-deployment、source-hash lookupを行わない。
...
## 2026-07-29 — Native Windows parallel Shogun Workspace contract (Task 9-B)
...
- Bound activation to a later one-time deployment checkpoint covering the
  ...
  not activate or deploy the contract.
- Preserved task-scoped user approval: one approved sealed manifest for launch,
  active-Shogun ownership for review, and exact-invocation approval for cancel.
  There is no per-worker, per-turn, or tool-call approval loop.
- Preserved one to seven direct workers and reviewer ownership by the active
  Codex or Claude Shogun. CLI version evidence covers only the installed and
  required CLI set under the repository `pinned_versions_for` contract.
