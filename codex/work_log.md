...
- 専用スキルや新しい常時読込ファイルは追加していない。Shogun側は独立repoの `CLAUDE.md` と `instructions/common/protocol.md` を正本として同等の最小ルールを追加し、repo標準の生成処理で `AGENTS.md` と各CLI・役職向けMarkdownを同期した。runtime・tmux・テスト運転は変更・実行していない。
