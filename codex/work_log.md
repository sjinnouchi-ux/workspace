...
- Kアラート開発を1からCodexで継続支援する
- 設計メモだけでなく、GAS/Worker/手順を含む実装プロジェクトとして分けて管理したい

### 対応内容

- `k-alert-test/` プロジェクトを作成
- スプレッドシート設計、実装計画、手動セットアップ手順を作成
- GAS雛形とCloudflare Worker雛形を作成
- ルート `README.md` と `CLAUDE.md` のプロジェクト一覧に追加

### 残課題

- [ ] `yumekango.com` 側でスプレッドシートを作成
- [ ] GASをブラウザで作成し、雛形コードを貼り付ける
- [ ] Script Propertiesを設定する
- [ ] WorkerへKアラートルーティングを統合する
